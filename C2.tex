...
Thus we see that the worse case is the average case and that it is
asymptotically the only relevant one. In this situation, one needs
...
expensive one and takes $\Theta(p^{3k})$ operations in $\F_q$, that is
$\Omega(p^{3k}d) = \Omega(\ell^3d)$ operations in $\F_p$, plus a
storage of $\Theta(\ell^2d)$ elements of $\F_p$. Observe that this
precomputation may be used to compute any other isogeny.
...
interpolation phase. Using fast techniques, this can be done in
$\Theta(\Mult(p^k)\log p^k)$ operations in $\U_k$, that is at least
$\Omega(p^kd\Mult(p^k)\log p^k) = \Omega(\ell^2d\log
\ell)$. Mistakingly, the $p^kd$ contribution due to arithmetics in
$\U_k$ had been underestimated in the complexity analysis of
\cite{Cou96}, thus leading to the wrong complexity estimate of
$\Omega(\ell d\log\ell)$ for this phase.
