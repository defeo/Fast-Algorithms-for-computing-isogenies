...
doesn't suppose $p$ to be fixed. In Section \ref{sec:bounded} we also
extend the algorithm beyond its original scope to solve the more
general problem of \emph{smallest degree isogeny finding};
interestingly, this new algorithm turns out to have a better
...
$f,g,h\in\K[X]$ of degree at most $n$. The most commonly used
algorithm for modular composition is \cite{BrKu78}, which implies
$\ModComp(n)\in O\left(n^{\frac{\omega+1}{2}}\right)$; the recent work
of Umans \cite{Uma09} shows that $\ModComp(n)\in
O\left(n^{1+\varepsilon}\right)$ for any $\varepsilon>0$, however this
algorithm doesn't seem to be efficient in practice.
